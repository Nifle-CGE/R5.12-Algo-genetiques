...
def algorithme_genetique(
    ...
    while time.perf_counter() - start < temps_dexecution:
        # if montrer_evolution and (gen % (generations // 10) == 0 or gen == generations - 1):
        #     meilleur_tour = min(population, key=lambda tour: distance_totale(villes, tour))
        #     print(f"Génération {gen} : Meilleure distance totale = {distance_totale(villes, meilleur_tour)}")
        #     visualiser_villes(villes, meilleur_tour)
    ...
    meilleur_tour = min(population, key=lambda tour: distance_totale(villes, tour))
    return meilleur_tour, distance_totale(villes, meilleur_tour), generations
